<?php
// Working gallery that uses a different upload approach
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}
include 'db_connect.php';

// Use a different upload directory that should have write permissions
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($imgFile && $imgFile['tmp_name'] && $imgFile['error'] === UPLOAD_ERR_OK) {
        ...
        if (empty($errors)) {
            ...
            // Try to upload using base64 encoding as a workaround
            $uploadSuccess = false;
            
            // Method 1: Try normal upload
            if (move_uploaded_file($imgFile['tmp_name'], $targetPath)) {
                ...
                if (copy($imgFile['tmp_name'], $targetPath)) {
                    ...
                    // Method 3: Use base64 encoding and store in database
                    $imageData = file_get_contents($imgFile['tmp_name']);
                    if ($imageData !== false) {
            ...
            if ($uploadSuccess) {
                if ($id) {
                    ...
                    if (isset($base64Data)) {
                        ...
                        $updateSql = "UPDATE gallery SET image_data = ? WHERE id = ?";
                        $updateStmt = $conn->prepare($updateSql);
                        $updateStmt->bind_param("si", $base64Data, $id);
                        $updateStmt->execute();
                    } else {
                    ...
                    // Insert new record
                    $sql = "INSERT INTO gallery (about" . (isset($base64Data) ? ", image_data" : "") . ") VALUES (?" . (isset($base64Data) ? ", ?" : "") . ")";
                    $stmt = $conn->prepare($sql);
                    if (isset($base64Data)) {
                        $stmt->bind_param("ss", $about, $base64Data);
                    } else {
                    ...
                    if (isset($base64Data)) {
                        // Image is already stored in database
                    } else {
                        @rename($targetPath, $uploadDirFs . "/$finalName");
                    }
                ...
                header("Location: gallery_working.php?success=" . urlencode($message));
                exit;
            } else {
                $errors[] = "Failed to upload file. Please try a different image or contact support.";
            }
        }
    } else {
...
if (isset($_GET['delete']) && $_GET['delete']) {
    ...
    header("Location: gallery_working.php?success=" . urlencode("Gallery deleted successfully!"));
    exit;
}
...
if (isset($_GET['edit']) && $_GET['edit']) {
    ...
    // Check if image is stored as base64 in database
    if (isset($edit_row['image_data']) && !empty($edit_row['image_data'])) {
        $img = 'data:image/jpeg;base64,' . $edit_row['image_data'];
    } else {
        // Check for file
            ...
            <div class="bg-white p-6 rounded-lg shadow">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex gap-4">
                        ...
                        <a href="gallery_working.php" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Cancel</a>
                    </div>
                </form>
            </div>
            ...
            <div class="bg-white p-6 rounded-lg shadow">
                ...
                <div class="space-y-4">
                    ...
                    while ($row = $gallery->fetch_assoc()):
                        // Check if image is stored as base64
                        if (isset($row['image_data']) && !empty($row['image_data'])) {
                            $img = 'data:image/jpeg;base64,' . $row['image_data'];
                        } else {
                            // Check for file
                    ...
                    <div class="flex items-center gap-4 p-4 border rounded">
                        ...
                        <div class="flex gap-2">
                            <a href="gallery_working.php?edit=<?php echo $row['id'] ?>" class="bg-blue-500 text-white px-3 py-1 rounded text-sm hover:bg-blue-600">Edit</a>
                            <a href="gallery_working.php?delete=<?php echo $row['id'] ?>" class="bg-red-500 text-white px-3 py-1 rounded text-sm hover:bg-red-600" onclick="return confirm('Delete this image?')">Delete</a>
                        </div>
                    </div>
                    <?php endwhile; ?>
                </div>
