...
Class Action {
	...
	function save_settings(){
		...
		// System settings feature removed
	}
	...
	function delete_alumni(){
		...
		$id = intval($id);
		if(!$id) return 0;
		// Temporary logging to help debug duplicate requests
		$log_file = __DIR__ . '/delete_log.txt';
		@file_put_contents($log_file, "[".date('Y-m-d H:i:s')."] delete_alumni called with id={$id}\n", FILE_APPEND);
		// Start transaction to ensure consistency
		$this->db->begin_transaction();
		// Remove any user account linked to this alumnus
		$del_user = $this->db->query("DELETE FROM users WHERE alumnus_id = $id");
		// Delete the alumnus bio
		$del_bio = $this->db->query("DELETE FROM alumnus_bio WHERE id = $id");
		if($del_bio){
			$this->db->commit();
			@file_put_contents($log_file, "[".date('Y-m-d H:i:s')."] delete_alumni success for id={$id}\n", FILE_APPEND);
			// return JSON string; ajax.php will echo this
			return json_encode(['success' => 1]);
		} else {
			// Rollback and return DB error for debugging (remove or sanitize in production)
			$error = $this->db->error;
			$this->db->rollback();
			@file_put_contents($log_file, "[".date('Y-m-d H:i:s')."] delete_alumni FAILED for id={$id} error={$error}\n", FILE_APPEND);
	...
	function save_forum(){
		...
		// Check for duplicate topic title for the same user
		$chk = $this->db->query("SELECT id FROM forum_topics WHERE title = '$title' AND user_id = '$user_id'");
